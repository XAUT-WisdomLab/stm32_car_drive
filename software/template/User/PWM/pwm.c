...
void PWM_Init(void)
{
    //-----------舵机的PWM初始化-----------
    // 使能定时器的通道
    HAL_TIM_PWM_Start(&TIM_3,PWM_SERVO_CHANNEL_A);
    HAL_TIM_PWM_Start(&TIM_3,PWM_SERVO_CHANNEL_B);
    // 设置初始占空比为0
    __HAL_TIM_SET_COMPARE(&TIM_3, PWM_SERVO_CHANNEL_A, 0);
    __HAL_TIM_SET_COMPARE(&TIM_3, PWM_SERVO_CHANNEL_B, 0);
} 
...
void SERVO_PWMA_Set(uint16_t pwm_d)
{
    ...
    __HAL_TIM_SET_COMPARE(&TIM_3, PWM_SERVO_CHANNEL_A, pwm_d);
}
...
void SERVO_PWMB_Set(uint16_t pwm_d)
{
    ...
    __HAL_TIM_SET_COMPARE(&TIM_3, PWM_SERVO_CHANNEL_B, pwm_d);
}
